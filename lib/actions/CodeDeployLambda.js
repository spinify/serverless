...
module.exports   = function(S) {
  ...
  class Deployer {
    ...
    _compress() {
      ...
      return new BbPromise(function(resolve, reject) {
        ...
        .then(function () {
          ...
          return _this._isZipAvailable()
        })
        .then(function (isZipAvailable) {
          if (isZipAvailable) {
            return _this._compressNativeZip(files)
          }

          return _this._compressNodeZip(files)
        })
        .then(function () {
    ...
    _compressNativeZip() {
      SUtils.sDebug('Compressing ZIP using native command');
      let _this = this;

      const zipCommand = `zip --recurse-paths --quiet ../${this.zipName} *`;

      return new BbPromise((resolve, reject) => {
        exec(zipCommand, {
          cwd: _this.evt.options.pathDist
        }, (error, stdout, stderr) => {
          _this.zipBuffer = fs.readFileSync(_this.pathCompressed);

          return error || stderr
            ? reject(error)
            : resolve(_this.zipName);
        });
      });
    }

    _compressNodeZip(files) {
      SUtils.sDebug('Compressing ZIP using Node.JS library');

      let zip = new Zip();
      files.forEach(function (file) {
        zip.file(file.name, fs.readFileSync(file.item.path), {
          unixPermissions: fs.statSync(file.item.path).mode | 0o444
        })
      });
